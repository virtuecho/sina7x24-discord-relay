...
- 用 D1 持久化 relay 游标、最新一次运行摘要，以及最小化的 itemId -> messageId 映射
- 自动删除超过 7 天的 `relay_items` 记录
- 提供状态查看和手动执行的管理接口
...
- `src/store.js` — D1 中的游标、最新一次运行摘要和最小消息映射存储
...
Worker 只保留最新一次运行摘要和最小化的 relay 映射，并会删除超过 7 天的 `relay_items` 记录。
